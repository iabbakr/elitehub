...
export async function generateMetadata({ params }: AppPageProps<{ serviceId: string }>): Promise<Metadata> {
...
export default async function ServiceProviderProfilePage({ params }: AppPageProps<{ serviceId: string }>) {
    const provider = await fetchServiceProviderById(params.serviceId);
    if (!provider) {
        notFound();
    }
