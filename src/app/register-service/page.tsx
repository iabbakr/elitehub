...
import { nigerianStates, serviceCategories, serviceSubCategories, checkIfEmailExists } from '@/lib/data';
import { Checkbox } from '@/components/ui/checkbox';
import Link from 'next/link';
import { useAuth } from '@/hooks/use-auth';
import { useRouter } from 'next/navigation';
import { createUserWithEmailAndPassword, signInWithEmailAndPassword, updateProfile } from 'firebase/auth';
import { handleReferralOnSignup } from '../actions/adminActions';
...
export default function RegisterServicePage() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      rcNumber: '', bio: '', city: '', operatesOnline: false, hasPhysicalLocation: false,
      address: '', terms: false, referralCode: ''
    },
  });
    ...
    <div className="flex justify-center items-center py-12">
      <Card className="w-full max-w-xl shadow-xl">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>
            {user ? "You are already signed in. Fill out the form below to apply." : "Join our network of trusted professionals."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <FormField control={form.control} name="serviceCategory" render={({ field }) => (<FormItem><FormLabel>Service Category</FormLabel><FormControl><Select onValueChange={field.onChange} defaultValue={field.value}><SelectTrigger><SelectValue placeholder="Select your main category" /></SelectTrigger><SelectContent>{serviceCategories.map((cat) => (<SelectItem key={cat.id} value={cat.name}>{cat.name}</SelectItem>))}</SelectContent></Select></FormControl><FormMessage /></FormItem>)}/>
                 <FormField control={form.control} name="serviceType" render={({ field }) => (<FormItem><FormLabel>Specific Service</FormLabel><FormControl><Select onValueChange={field.onChange} defaultValue={field.value} disabled={!selectedCategory}><SelectTrigger><SelectValue placeholder={selectedCategory ? "Select your service" : "Select category first"} /></SelectTrigger><SelectContent>{(serviceSubCategories[selectedCategory] || []).map((subCat) => (<SelectItem key={subCat} value={subCat}>{subCat}</SelectItem>))}</SelectContent></Select></FormControl><FormMessage /></FormItem>)}/>
              </div>
