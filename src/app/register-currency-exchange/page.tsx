    ...
    <div className="flex justify-center items-center py-12">
      <Card className="w-full max-w-xl shadow-xl">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>
            Join our network of trusted exchange agents.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                        ...
                        <FormItem>
                            ...
                            <div className="flex gap-4 pt-2">
                                {['Fiat', 'Crypto'].map(item => (
                                   <FormField key={item} control={form.control} name="currenciesAccepted" render={({ field }) => (
                        ...
                        <FormItem>
                            ...
                            <div className="grid grid-cols-2 gap-2 pt-2">
                                {transactionTypes.map(item => (
                                   <FormField key={item} control={form.control} name="transactionTypes" render={({ field }) => (
                                        <FormItem key={item} className="flex flex-row items-start space-x-3 space-y-0">
                                            <FormControl><Checkbox checked={field.value?.includes(item)} onCheckedChange={(checked) => { return checked ? field.onChange([...(field.value || []), item]) : field.onChange(field.value?.filter((value) => value !== item)) }} /></FormControl>
                                            <FormLabel className="font-normal">{item}</FormLabel>
                                        </FormItem>
