...
import { ArrowRight, Wrench, Briefcase, Code, Brush, Video, Home, GraduationCap, Heart, Car, Shield, PartyPopper } from 'lucide-react';
import { serviceCategories } from '@/lib/data';

const iconMap: { [key: string]: React.ElementType } = {
  'professional-services': Briefcase,
  'digital-services': Code,
  'creative-media-services': Brush,
  'home-repair-services': Home,
  'education-training': GraduationCap,
  'beauty-personal-care': Heart,
  'automobile-services': Car,
  'security-services': Shield,
  'event-travel-services': PartyPopper,
};
    ...
    <div>
      <header className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-muted-foreground max-w-3xl mx-auto">
          Choose a service category to find reliable and verified professionals for your needs.
        </p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
        {serviceCategories.map((category) => {
          const Icon = iconMap[category.id] || Wrench;
          return (
            <Card key={category.id} className="text-center hover:shadow-xl transition-shadow">
              ...
              <CardContent>
                <Link href={`/category/services/${category.id}`} passHref>
                  <Button>
