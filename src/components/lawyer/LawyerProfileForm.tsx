...
import { nigerianStates, practiceAreas, type Lawyer, createProfileUpdateRequest } from '@/lib/data';
...
export function LawyerProfileForm({ lawyer, onSuccess, onCancel }: LawyerProfileFormProps) {
  ...
  const canRequestUpdate = useMemo(() => {
    if (lawyer.profileUpdateStatus === 'pending') return false;
    if (!lawyer.lastProfileUpdateRequest) return true;
    const lastRequestDate = new Date(lawyer.lastProfileUpdateRequest.seconds * 1000);
    return differenceInDays(new Date(), lastRequestDate) >= 30;
  }, [lawyer.profileUpdateStatus, lawyer.lastProfileUpdateRequest]);

  const daysUntilNextUpdate = useMemo(() => {
    if (!lawyer.lastProfileUpdateRequest) return 0;
    const lastRequestDate = new Date(lawyer.lastProfileUpdateRequest.seconds * 1000);
    const daysSince = differenceInDays(new Date(), lastRequestDate);
    return 30 - daysSince;
  }, [lawyer.lastProfileUpdateRequest]);
  ...
  async function onSubmit(values: z.infer<typeof profileFormSchema>) {
    ...
    try {
      await createProfileUpdateRequest('lawyer', lawyer.id, lawyer.uid!, values);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow min-h-0">
          <ScrollArea className="flex-grow px-6 py-4">
            <div className="space-y-4">
                {lawyer.profileUpdateStatus === 'pending' && (
                ...
                {lawyer.profileUpdateStatus === 'approved' && (
                ...
                {!canRequestUpdate && lawyer.profileUpdateStatus !== 'pending' && (
